...
Page({
	...
	getArticleIndex: function () {
		...
		api.article_index({
			...
			if(data.status == 200){
				self.setData({
					article_index:data.data.cardList[0].list
				})
				console.log(self.data.article_index)
			}
		})
	},
	...
	goPhotoDetail: function (e) {
		var activeid = e.currentTarget.dataset.item.activeInfo.activeid
		wx.navigateTo({
			url: '/pages/photo-detail/photo-detail?activeid='+activeid
		})
	},
	addPhoto: function () {
		var self = this
		wx.chooseImage({
			...
			success: function(e) {
				getApp().globalData.AppPhotosTemp = {
					type: "news",
					temp: e.tempFilePaths,
					card_info:'1'
				}
				wx.navigateTo({
